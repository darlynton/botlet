            ...
            try:
                ...
                for reminder in due_reminders:
                    ...
                    if send_result and send_result.get("status") == "success":
                        ...
                        if mark_reminder_sent(reminder["id"]):
                            if self._should_log():
                                print(f"Marked reminder {reminder['id']} as complete")
                    else:
                        if self._should_log():
                            print(f"Failed to send or confirm reminder {reminder['id']}. Result: {send_result}")
                        # Ensure the reminder is not marked as sent if sending fails
            except Exception as e:
